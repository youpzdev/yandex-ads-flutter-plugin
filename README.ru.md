...
* следит, чтобы одновременно показывалась только одна полноэкранная реклама —
  во всём процессе, а не в одном пуле;
...
* когда бюджет попыток исчерпан, переходит в `FullscreenAdPoolStatus.exhausted`
  и ждёт `retry()`;
* выбрасывает объявления, запрошенные до того, как `setUserConsent` или
  `setAgeRestricted` изменили ответ пользователя.
...
* Android проверен сборкой и запуском примера на устройстве; реальный fill,
  показы и клики на боевых блоках — нет.
