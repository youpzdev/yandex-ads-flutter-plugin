...
final class AppOpenAdLoaderCommandProvider: LoadDelegate, CommandProvider {
    ...
    init(adLoader: AppOpenAdLoader, loadDelegate: FullscreenAdLoadDelegate, onDestroy: @escaping () -> Void) {
    ...
    private func loadAppOpenAd(args: Any?, result: MethodCallResult) {
        ...
        adLoader.loadAd(with: adRequest) { [weak self] loadResult in
            switch loadResult {
            case .success(let ad):
                self?.loadDelegate.reportAppOpenAdLoaded(ad: ad, adInfo: ad.adInfo, requestId: requestId)
